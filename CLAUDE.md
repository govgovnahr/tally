...
- **Frontend** (`client/src/**/*.test.{js,jsx}`, Vitest + `@testing-library/react` + jsdom): colocated with the source file, not a separate `__tests__` tree. `ColorsContext` (`colors.jsx`) has a default value, so components using `useC()` render without a provider wrapper in tests. `glasscn-ui`'s `Card` ships its own React copy that jsdom rejects ("element from an older version of React") — mock it (`vi.mock('glasscn-ui', ...)`) in any test that renders a component using it.
...
- **Parity-test pattern**: when a new endpoint aggregates/wraps logic that already exists elsewhere (e.g. `GET /dashboard` reusing the summary/pacing/outliers/goals helpers), add a test asserting the new endpoint's sections equal the original endpoints' output (see `test_dashboard.py`) — this is what catches the aggregating helper drifting from what it was factored out of.
...
- The **"Other" expense type specifically** cannot be deleted (`types_router.py` checks `is_default and name == 'Other'`, 403; frontend hides the button only for `name === 'Other'`) — not a general `is_default=1` rule. The other 10 seeded default types (see `_DEFAULT_TYPES` in `database.py`) are deletable like any custom type.
...
- `database.py`'s `get_connection()` draws from a module-level `psycopg2.pool.ThreadedConnectionPool` (min 2 / max 20), not a fresh `psycopg2.connect()` per call. `conn.close()` returns the connection to the pool (rolling back first if it wasn't committed) rather than actually closing the socket — don't add code that assumes `close()` terminates the connection. `close()` is idempotent. The pool is created lazily on first use and torn down in `server.py`'s `shutdown` event via `close_pool()`.
- `POST /incomes` rejects a same `user_id`+`name`+`amount` submission within 5s of a prior one (409) — a duplicate-submit guard, keyed loosely on purpose (ignores `date`) since a hand-retried submission may have a different date typed in. See `_DUPLICATE_WINDOW_SECONDS` in `incomes_router.py`.
- Backend tests: `server.py`'s global fixed-window rate limiter (100 writes/min, 300 GETs/min per key) is raised to 100k in `tests/conftest.py` — every `TestClient` request shares one fake rate-limit key, and a full test run can legitimately burst past the real-user limit.
...
- **Write endpoints prefer single-statement `RETURNING`** over separate check-then-write round trips: `INSERT ... WHERE EXISTS(...) RETURNING id` (type/existence validated in the insert itself), `DELETE ... RETURNING id` (existence check folded into the delete), `UPDATE ... RETURNING *` in place of a write followed by a read-back SELECT. `fetchone() is None` means the guard failed — always `conn.close()` *before* raising, so `_release` returns a clean connection to the pool. Where collapsing would blur a 404-vs-400 distinction (`update_expense` needs to tell "wrong id" from "bad type" apart), a lightweight validation query is kept instead of folding everything into one statement.
- `GET /dashboard?month=` is an aggregate endpoint for `DashboardPage.jsx` — one request instead of the ~8 separate summary/pacing/outliers/goals GETs it used to fire (and the `periodBounds` waterfall gating half of them). Its sections are computed by `conn`-taking helpers that the standalone endpoints (`/expenses/summary`, `/incomes/summary`, `/macrocategories/summary`, `/analysis/pacing`, `/analysis/outliers`, `/savings-goals`) also call, so there's one source of truth per section — `server/tests/test_dashboard.py` asserts parity. The frontend's `['dashboard']` React Query key isn't a child of `['expenses']`/`['incomes']`/`['analysis']`/etc., so any write that should refresh the dashboard needs its own explicit `queryClient.invalidateQueries({ queryKey: ['dashboard'] })` alongside the usual invalidations.
...
- **Rolled out to**: `compute_budget_pacing`/`_fast_totals_by_type` (database.py), `GET /analysis/pacing`, `GET /expenses/months`, and the Dashboard-only summary + list endpoints (`/expenses`, `/expenses/summary`, `/incomes`, `/incomes/summary`, `/macrocategories/summary`, `DELETE /transactions` — each also accepts optional `period_start`/`period_end`, preferred over `month` when present). `GET /dashboard` computes `cycle_bounds(month, cycle_start_day)` once server-side and passes the resulting `period_start`/`period_end` into every section helper it calls — it's cycle-aware by construction, not bolted on. Frontend: `MonthSelector`, `DashboardPage` (incl. its embedded `ExpenseList` and "Clear Month"), `AnalysisPage`'s pacing section, and `App.jsx`'s initial-month bootstrap all read period bounds from the server (`GET /settings/period-bounds?month=`, or `GET /settings/period-bounds-bulk?months=a,b,c` for showing several at once) rather than re-deriving cycle math client-side — `lib/budgetMonths.js` stays calendar-only on purpose.
...
**Budget pacing** (implemented): computed by `compute_pacing_payload` (`analysis_router.py`) — `projected = spent + historical_daily_rate × remaining_days`. Historical rate = total spent in past N months / total days in those months, per category. Past months return `projected_spend: null`. Exposed directly at `GET /analysis/pacing` (used by `AnalysisPage`) and via `GET /dashboard`'s `pacing` section (used by `DashboardPage`/`SummaryBar`, which receives it as a prop rather than fetching it itself) — both call the same helper, so there's one implementation. Two-tone progress bars: solid = actual (category color, red only if actually over), ghost extension = projected-beyond-actual (status color at 40% opacity).
