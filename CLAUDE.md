...
- `budget.db` lives at `~/.budget_app/budget.db`, not bundled
...
- New routers need `--hidden-import` in build workflow (both platforms)
...
### Multi-user
`user_id TEXT` stubs on `incomes` and `expense_types` (NULL everywhere). Future: `ALTER TABLE` migrations for all tables; `budgets` PK changes to `(user_id, type)`; JWT/session auth. Avoid global/singleton backend state.
