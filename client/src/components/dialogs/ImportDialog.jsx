...
export default function ImportDialog({ onClose, onImported }) {
  ...
  async function handleFetchSuggestions() {
    ...
    try {
      ...
      setSuggestions({ ...res.data, rows: res.data.rows.map(r => ({ ...r, id: String(r.row_idx) })) })
      setOverrides({})
    ...
    try {
      ...
      if (suggestions) {
        const confirmed = Object.fromEntries(
          suggestions.rows.map(r => [String(r.row_idx), overrides[r.id] ?? r.suggested_type])
        )
      ...
      <DialogContent className={step === 2 ? 'max-w-4xl' : 'max-w-lg'}>
        ...
        {step === 2 && suggestions && (
          <ReviewTable
            ...
            validTypes={suggestions.valid_types}
            incomeCount={suggestions.income_count}
            aiCapReached={suggestions.ai_cap_reached}
            aiEnabled={suggestions.ai_enabled}
            overrides={overrides}
